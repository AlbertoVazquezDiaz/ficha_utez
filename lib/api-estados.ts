...
export async function fetchEstadosMexico(): Promise<Estado[]> {
  const res = await fetch("http://localhost:8080/api/fichas-utez/states")
...
export async function fetchMunicipiosPorEstado(stateId: number): Promise<Municipio[]> {
  const res = await fetch(`http://localhost:8080/api/fichas-utez/municipalities/state/${stateId}`)
...
export async function fetchTiposPrepa(): Promise<TipoPrepa[]> {
  const res = await fetch("http://localhost:8080/api/fichas-utez/high-school-types")
